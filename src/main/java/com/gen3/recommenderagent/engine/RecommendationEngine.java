...
public class RecommendationEngine {
  ...
  public Recommendations process(String sessionId, String userId, SessionRequest currentRequest) {
    ...
    switch (intent) {
      ...
      case REFINE_RECOMMENDATION:
        recommendations = handleRefineRecommendation(currentRequest, session);
        break;

      case FILTER_BY_LENGTH:
        recommendations = handleFilterByLength(currentRequest, session);
        break;

      case FILTER_BY_NARRATOR:
        recommendations = handleFilterByNarrator(currentRequest, session);
        break;
      ...
      case MORE_RESULTS:
        recommendations = handleMoreResults(currentRequest, session);
        break;

      case CHANGE_COUNT:
        recommendations = handleChangeCount(currentRequest, session);
        break;
      ...
      case REJECT_RECOMMENDATIONS:
        recommendations = handleRejectRecommendations(currentRequest, session);
        break;

      case ALREADY_READ:
        recommendations = handleAlreadyRead(currentRequest, session);
        break;

      case LIKE_RECOMMENDATION:
        recommendations = handleLikeRecommendation(currentRequest, session);
        break;
      ...
      case BOOK_DETAILS:
        recommendations = handleBookDetails(currentRequest, session);
        break;
      ...
      case UPDATE_PREFERENCES:
        recommendations = handleUpdatePreferences(currentRequest, session);
        break;

      case CLEAR_HISTORY:
        recommendations = handleClearHistory(currentRequest, session);
        break;
    ...
    eventPublisher.publishEvent(new SessionUpdateEvent(session));
  ...
  private Recommendations handleNewRecommendation(SessionRequest request) {
  ...
  private Recommendations handleSimilarToBook(SessionRequest request) {
  ...
  private Recommendations handleSimilarToAuthor(SessionRequest request) {
  ...
  private Recommendations handleSimilarToNarrator(SessionRequest request) {
  ...
  private Recommendations handleRefineRecommendation(SessionRequest request, Session session) {
  ...
  private Recommendations handleFilterByLength(SessionRequest request, Session session) {
  ...
  private Recommendations handleFilterByNarrator(SessionRequest request, Session session) {
  ...
  private Recommendations handleMoreResults(SessionRequest request, Session session) {
  ...
  private Recommendations handleChangeCount(SessionRequest request, Session session) {
  ...
  private Recommendations handleRejectRecommendations(SessionRequest request, Session session) {
  ...
  private Recommendations handleAlreadyRead(SessionRequest request, Session session) {
  ...
  private Recommendations handleLikeRecommendation(SessionRequest request, Session session) {
  ...
  private Recommendations handleBookDetails(SessionRequest request, Session session) {
  ...
  private Recommendations handleUpdatePreferences(SessionRequest request, Session session) {
  ...
  private Recommendations handleClearHistory(SessionRequest request, Session session) {
  ...
  private Recommendations handleGreeting(SessionRequest request) {
  ...
  private Recommendations handleHelp(SessionRequest request) {
  ...
  private Recommendations handleUnknown(SessionRequest request) {
  ...
  private String buildRecommendationQuery(SessionRequest request) {
  ...
  private int resolveResultLimit(SessionRequest request) {
    ...
    return Math.min(Math.max(request.getConstraints().getCount(), 1), MAX_RESULT_LIMIT);
  }
}
